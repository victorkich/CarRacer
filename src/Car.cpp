...
public:
    ...
    void updateProgress() {
        ...
        for (int i = 0; i < 5; i++)
            if (progress[i])
                index++;
        if (20 * (index + 1) >= position && position >= index * 20)
            progress[index] = true;
    ...
    QSMatrix<float> Render(bool show_lasers) {
        ...
        QSMatrix<float> mat_obs(10, 1, 0);
        for (unsigned j = 0; j < 8; j++) {
            mat_obs.modify(j, 0, obs[j]);
        }
        mat_obs.modify(8, 0, get_position());
        mat_obs.modify(9, 0, float(get_speed()));
        return mat_obs;
    }
};